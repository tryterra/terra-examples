...
export class StreamingConsumer {
  ...
  private onClose(ev: CloseEvent): void {
    ...
    if (ev.code === CloseCode.SESSION_LIMIT) {
      // Every consumer slot is taken (the cap is currently 10 per
      // developer). Auto-retrying here wouldn't free one, and if this client
      // is one of the holders it would just kick itself off — surface it and
      // let the user retry manually.
      this.fail(
        "The consumer limit (10 per developer) is reached (close 4002). " +
          "Close other tabs or disconnect a backend consumer, then retry.",
      );
      return;
    }
